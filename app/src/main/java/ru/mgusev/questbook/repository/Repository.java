...
public class Repository {
    ...
    public void loadGameFromDB() {
        Game loadedGame = questBookDB.gameDAO().getGame();
        if (loadedGame != null) {
            loadedGame.setCharacteristicList(questBookDB.userCharacteristicDAO().getUserCharacteristics(loadedGame.getId()));
            for (UserCharacteristic c : loadedGame.getCharacteristicList()) {
                System.out.println(c.getCharacteristicId() + " " + c.getCount());
            }
